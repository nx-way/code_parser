...
namespace
{
			...
			CXCursor cursor, CXCursor parent, CXClientData client_data)
	{
		...
		{
			std::string name = parser::Convert(clang_getCursorSpelling(cursor));
...
{
	Enum e{GetFile(cursor), GetFullName(cursor), GetComment(cursor), GetCode(cursor,TypeBase::Type::Enum )};
