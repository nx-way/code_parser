...
#undef SPLIT_STRING
/*
 const char* str = "[json]:[toml]";
        char* x;
        int idx = 0;
        SPLIT_STRING(str,":",idx, x, { printf("-- str[%i]: %s\n",idx, x);  if(strcmp(x,"[json]") == 0){ printf("is_json\n");} });
 */
#define SPLIT_STRING(st, delim, idx, x, lambda_run) {\
char SPLIT_STRING_dup[500] = {0};                \
strcat(SPLIT_STRING_dup,st) ;                                                 \
...
#define SPLIT_STRING_DUP(st,delim,idx, x, lambda_run ) {\
